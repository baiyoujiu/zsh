        ...
        for(var key in data){
            ...
            if(data[key].type == 'text'){
                ...
                if(isSecKill == '1'){
                    ...
                    //库存增加个自定义的字段，以此来控制只增不减
                    if(data[key].name == 'stock'){
                        input.setAttribute('data-stock','readonly');
                    }
                }
    ...
    var setJson = function(data){
        ...
        arr.forEach(function(obj,i){
            ...
            $(JR).eq(i).find('input[class^="J_map"]').each(function(){
                ...
                this.value = val;
                $(this).attr('data-stock',val);
            });
            if(isSecKill == '1'){
                $("input.J_mapstock").blur(function () {
                    var oldStock = Number($(this).attr("data-stock"));
                    var newStock = Number($(this).val());
                    if(isNaN(newStock)){
                        $(this).val(oldStock);
                        return;
                    }
                    if(newStock < oldStock){
                        //$.dialog.tips('该规格的库存不能少于原来的库存'+oldStock);
                        toastTips(2, '该规格的库存不能少于原来的库存'+oldStock, "", 2000, false);
                        $(this).val(oldStock);
                    }
                })
            }
...
;(function (window, jQuery, undefined) {
    ...
    function _bindEvent(){
        $('.shop-amend .info button').on('click',function(){
            if(isSecKill == '1'){
                toastTips(2, "限购活动期间，不能批量设置价格和库存〜", "", 2000, false);
                return;
            }
