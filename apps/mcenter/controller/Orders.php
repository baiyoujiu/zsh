...
class Orders extends Controller
{
	...
	*/
	...
	public function daihuan(){
        ...
        $statusbrr = ['0'=>'未还','1'=>'已还'];
        $this->assign('statusbrr',$statusbrr);
		return view();
	}
...
//归还时间？		$data['updatetime'] = date('Y-m-d H:i:s');
		$gno = $data['objId'];
		unset($data['objId']);
		unset($data['i']);
        $data['status'] = 1;
		$res = db('order_goods')->where('order_no',$gno)->update($data);
		if($res){
			return ['status'=>200,'msg'=>'成功！'];
		}else{
			return ['status'=>201,'msg'=>'不成功！'];
		...
		try{
			...
			foreach($glists as $v){
				...
				if($v['rent'] == 1){
					...
					$ugdata = ['rentstart'=>$rentstart,'rentend'=>$rentend,'addtime'=>$nowtimes];
