...
(function() {
	...
	MobileSelect.prototype = {
		...
		renderWheels: function(wheelsData, cancelBtnText, ensureBtnText){
			...
			//鍒�
				...
				if(_this.jsonType){
					...
					//琛�
						tempHTML += '<li data-id="'+wheelsData[i].data[j][_this.keyMap.id]+'"><img src="'+wheelsData[i].data[j][_this.keyMap.value]+'" /></li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){ 
					//琛�
						tempHTML += '<li><img src="'+wheelsData[i].data[j]+'" /></li>';
					}
				}
				tempHTML += '</ul></div>';
			}
		...
		checkArrDeep: function (parent) { 
			//妫€娴嬪瓙鑺傜偣娣卞害  淇敼 displayJson
			var _this = this;
			if(parent){
				if (_this.keyMap.childs in parent && parent[_this.keyMap.childs].length > 0) {
					_this.displayJson.push(_this.generateArrData(parent[_this.keyMap.childs])); //鐢熸垚瀛愯妭鐐规暟缁�
					_this.checkArrDeep(parent[_this.keyMap.childs][0]);//妫€娴嬩笅涓€涓瓙鑺傜偣
				}
			}
		},
